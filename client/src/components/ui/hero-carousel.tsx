...
export function HeroCarousel({
  ...
  const accent = active.accent ?? "#c5a572";
  ...
  return (
    ...
    >
      ...
      >
        {onBack ? (
          <button
            type="button"
            onClick={onBack}
            className="opacity-90 transition-opacity hover:opacity-100 flex items-center gap-1 cursor-pointer text-white/90 hover:text-gold"
        ...
        <div className="flex w-full flex-wrap items-end gap-x-[4vw] gap-y-2">
          ...
          {active.meta?.length ? (
            <div
              ...
              style={{ gap: `${Math.max(14, box.w * 0.035)}px` }}
            >
              {active.meta.map((fact, i) => (
                <motion.span
                  key={`${index}-${fact}`}
                  className="font-sans text-[11px] whitespace-nowrap uppercase tracking-[0.16em] text-white/80 bg-black/40 backdrop-blur-md px-3 py-1 rounded-full border border-white/10"
                  style={{ fontSize: Math.max(10, label) }}
      ...
      <div
        ...
        style={{ left: pad, bottom: Math.max(14, box.h * 0.022), width: Math.max(160, box.w * RAIL) }}
      >
        <div
          className="flex justify-between font-mono tabular-nums text-white/80"
